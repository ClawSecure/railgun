...
This guide covers how to create your own Railgun workflow.
...
Railgun runs workflow agents as isolated cron jobs in OpenClaw. You can override the
...
If omitted, Railgun defaults to 30 minutes per agent session.
...
Railgun includes shared agents in `agents/shared/` that you can reuse:
...
```bash
clawsecure-railgun workflow install my-workflow
```
