...
export function checkOrphanedCrons(
  ...
  // Extract unique workflow IDs from railgun cron job names
  const workflowIds = new Set<string>();
  for (const job of cronJobs) {
    const match = job.name.match(/^railgun\/([^/]+)\//);
  ...
  for (const wfId of workflowIds) {
    ...
    if (active.cnt === 0) {
      const jobCount = cronJobs.filter(j => j.name.startsWith(`railgun/${wfId}/`)).length;
      findings.push({
