...
import { resolveRailgunCli } from "../installer/paths.js";
import { readOpenClawConfig, writeOpenClawConfig } from "../installer/openclaw-config.js";

const MEDIC_CRON_NAME = "railgun/medic";
...
function buildMedicPrompt(): string {
  ...
  return `You are the Railgun Medic — a health watchdog for workflow runs.
...
\`\`\`
sessions_send(sessionKey: "agent:main:main", message: "🚑 Railgun Medic Alert: <summary of critical issues>")
\`\`\`
...
async function ensureMedicAgent(): Promise<void> {
  try {
    ...
    if (agents.some((a: any) => a.id === "railgun-medic")) return;
...
async function removeMedicAgent(): Promise<void> {
  try {
    const { path, config } = await readOpenClawConfig();
    const agents = config.agents?.list ?? [];
    const idx = agents.findIndex((a: any) => a.id === "railgun-medic");
    if (idx === -1) return;
    agents.splice(idx, 1);
    await writeOpenClawConfig(path, config);
  } catch {
