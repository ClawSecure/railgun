...
import { resolveRailgunCli } from "./paths.js";
...
function buildAgentPrompt(workflowId: string, agentId: string): string {
  ...
  const cli = resolveRailgunCli();

  return `You are a Railgun workflow agent. Check for pending work and execute it.
...
\`\`\`
cat <<'RAILGUN_EOF' > /tmp/railgun-step-output.txt
STATUS: done
CHANGES: what you did
TESTS: what tests you ran
RAILGUN_EOF
cat /tmp/railgun-step-output.txt | node ${cli} step complete "<stepId>"
\`\`\`
...
export function buildWorkPrompt(workflowId: string, agentId: string): string {
  ...
  return `You are a Railgun workflow agent. Execute the pending work below.
...
export async function setupAgentCrons(workflow: WorkflowSpec): Promise<void> {
  ...
  for (let i = 0; i < agents.length; i++) {
    ...
    const cronName = `railgun/${workflow.id}/${agent.id}`;
...
export async function removeAgentCrons(workflowId: string): Promise<void> {
  await deleteAgentCronJobs(`railgun/${workflowId}/`);
}
...
async function workflowCronsExist(workflowId: string): Promise<boolean> {
  ...
  const prefix = `railgun/${workflowId}/`;
