...
const BINARY_NAME = "clawsecure-railgun";

/**
 * Ensure `clawsecure-railgun` is available on PATH by symlinking into ~/.local/bin.
 * Safe to call repeatedly -- skips if already correct, updates if stale.
 * Skipped entirely in Docker mode (RAILGUN_SKIP_SYMLINK=1).
 */
export function ensureCliSymlink(): void {
  if (process.env.RAILGUN_SKIP_SYMLINK === "1") {
    console.log("  Symlink skipped (Docker mode: RAILGUN_SKIP_SYMLINK=1)");
    return;
  }
  ...
  if (existsSync(linkPath)) {
    try {
      ...
      if (!stats.isSymbolicLink()) {
        ...
        console.warn(`  ⚠ ${linkPath} exists and is not a symlink — skipping (remove it manually to let railgun manage it)`);
        return;
      }
