...
export function cleanupAbandonedSteps(): void {
  ...
  const activeRuns = db.prepare(
    "SELECT id, workflow_id, created_at FROM runs WHERE status = 'running'"
  ).all() as { id: string; workflow_id: string; created_at: string }[];
  ...
  const runningStepsWithStart = db.prepare(
    "SELECT s.id, s.step_id, s.run_id, s.retry_count, s.max_retries, s.type, s.current_story_id, s.loop_config, s.abandoned_count, s.started_at, r.workflow_id FROM steps s JOIN runs r ON r.id = s.run_id WHERE s.status = 'running' AND s.started_at IS NOT NULL AND r.status = 'running'"
  ).all() as { id: string; step_id: string; run_id: string; retry_count: number; max_retries: number; type: string; current_story_id: string | null; loop_config: string | null; abandoned_count: number; started_at: string; workflow_id: string }[];

  // Cache workflow limits to avoid re-reading YAML for every step in the same workflow
  const limitsCache = new Map<string, { maxRunSeconds: number; maxStepSeconds: number }>();
  ...
  for (const step of runningStepsWithStart) {
    ...
    if (elapsedMs <= limits.maxStepSeconds * 1000) continue;
    ...
    if (newAbandonCount >= MAX_ABANDON_RESETS) {
      ...
      ).run(detail, newAbandonCount, step.id);
      db.prepare(
        "UPDATE runs SET status = 'failed', updated_at = datetime('now') WHERE id = ?"
      ).run(step.run_id);
 ...
 */
function getWorkflowRuntimeLimits(workflowId: string): { maxRunSeconds: number; maxStepSeconds: number } {
 ...
 */
function failRunForTimeout(runId: string, maxRunSeconds: number): void {
  const db = getDb();
  const wfId = getWorkflowId(runId);
  ...
  db.prepare(
    "UPDATE steps SET status = 'failed', output = ?, updated_at = datetime('now') WHERE run_id = ? AND status IN ('pending', 'running')"
  ).run(message, runId);
...
export function claimStep(agentId: string): ClaimResult {
  ...
  const runMeta = db.prepare("SELECT workflow_id, created_at FROM runs WHERE id = ?").get(step.run_id) as { workflow_id: string; created_at: string } | undefined;
  if (runMeta) {
  ...
  if (step.type === "loop") {
    ...
    if (loopConfig?.over === "stories") {
      ...
      db.prepare(
        "UPDATE steps SET status = 'running', current_story_id = ?, started_at = COALESCE(started_at, datetime('now')), updated_at = datetime('now') WHERE id = ?"
  ...
  db.prepare(
    "UPDATE steps SET status = 'running', started_at = datetime('now'), updated_at = datetime('now') WHERE id = ? AND status = 'pending'"
  ).run(step.id);
...
export function completeStep(stepId: string, output: string): { advanced: boolean; runCompleted: boolean } {
  ...
  db.prepare(
    "UPDATE steps SET status = 'done', output = ?, completed_at = datetime('now'), updated_at = datetime('now') WHERE id = ?"
  ).run(output, stepId);
...
function checkLoopContinuation(runId: string, loopStepId: string): { advanced: boolean; runCompleted: boolean } {
  ...
  db.prepare(
    "UPDATE steps SET status = 'done', completed_at = datetime('now'), updated_at = datetime('now') WHERE id = ?"
  ).run(loopStepId);
...
export async function failStep(stepId: string, error: string): Promise<{ retrying: boolean; runFailed: boolean }> {
  ...
  if (newRetryCount > step.max_retries) {
    db.prepare(
      "UPDATE steps SET status = 'failed', output = ?, retry_count = ?, completed_at = datetime('now'), updated_at = datetime('now') WHERE id = ?"
