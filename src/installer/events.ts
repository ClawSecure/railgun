...
const EVENTS_DIR = path.join(os.homedir(), ".openclaw", "railgun");
...
export function emitEvent(evt: RailgunEvent): void {
  try {
...
function fireWebhook(evt: RailgunEvent): void {
...
export function getRecentEvents(limit = 50): RailgunEvent[] {
  try {
    const content = fs.readFileSync(EVENTS_FILE, "utf-8");
    const lines = content.trim().split("\n").filter(Boolean);
    const events: RailgunEvent[] = [];
    for (const line of lines) {
      try { events.push(JSON.parse(line) as RailgunEvent); } catch {}
    }
...
export function getRunEvents(runId: string, limit = 200): RailgunEvent[] {
  try {
    const content = fs.readFileSync(EVENTS_FILE, "utf-8");
    const lines = content.trim().split("\n").filter(Boolean);
    const events: RailgunEvent[] = [];
    for (const line of lines) {
      try {
        const evt = JSON.parse(line) as RailgunEvent;
