...
try {
  ...
  console.error(
    ...
    `Railgun requires Node.js >= 22 with native SQLite support.\n` +
    ...
    `  See: https://github.com/ClawSecure/railgun/issues/54`
  );
  process.exit(1);
}
...
import { getRecentEvents, getRunEvents, type RailgunEvent } from "../installer/events.js";
...
function formatEventLabel(evt: RailgunEvent): string {
...
function printEvents(events: RailgunEvent[]): void {
...
function printUsage() {
  process.stdout.write(
    [
      "railgun install                      Install all bundled workflows",
      "railgun uninstall [--force]          Full uninstall (workflows, agents, crons, DB)",
      "",
      "railgun workflow list                List available workflows",
      "railgun workflow install <name>      Install a workflow",
      "railgun workflow uninstall <name>    Uninstall a workflow (blocked if runs active)",
      "railgun workflow uninstall --all     Uninstall all workflows (--force to override)",
      "railgun workflow run <name> <task>   Start a workflow run",
      "railgun workflow status <query>      Check run status (task substring, run ID prefix)",
      "railgun workflow report <query>      Show duration report for a run",
      "railgun workflow runs                List all workflow runs",
      "railgun workflow resume <run-id>     Resume a failed run from where it left off",
      "railgun workflow stop <run-id>        Stop/cancel a running workflow",
      "railgun workflow ensure-crons <name>  Recreate agent crons for a workflow",
      "",
      "railgun dashboard [start] [--port N]   Start dashboard daemon (default: 3333)",
      "railgun dashboard stop                  Stop dashboard daemon",
      "railgun dashboard status                Check dashboard status",
      "",
      "railgun step peek <agent-id>        Lightweight check for pending work (HAS_WORK or NO_WORK)",
      "railgun step claim <agent-id>       Claim pending step, output resolved input as JSON",
      "railgun step complete <step-id>      Complete step (reads output from stdin)",
      "railgun step fail <step-id> <error>  Fail step with retry logic",
      "railgun step stories <run-id>       List stories for a run",
      "",
      "railgun medic install                Install medic watchdog cron",
      "railgun medic uninstall              Remove medic cron",
      "railgun medic run [--json]           Run medic check now (manual trigger)",
      "railgun medic status                 Show medic health summary",
      "railgun medic log [<count>]          Show recent medic check history",
      "",
      "railgun logs [<lines>]               Show recent activity (from events)",
      "railgun logs <run-id>                Show activity for a specific run",
      "",
      "railgun version                      Show installed version",
      "railgun update                       Pull latest, rebuild, and reinstall workflows",
    ].join("\n") + "\n",
  );
}
...
async function main() {
  ...
  if (group === "version" || group === "--version" || group === "-v") {
    console.log(`railgun v${getVersion()}`);
    return;
  }
  ...
  if (group === "update") {
    ...
    try {
      ...
      process.stderr.write("Failed to git pull. Are you in the railgun repo?\n");
      process.exit(1);
    }
  ...
  if (group === "uninstall" && (!args[1] || args[1] === "--force")) {
    ...
    console.log("Railgun fully uninstalled (workflows, agents, crons, database, skill).");
    return;
  }
  ...
  if (group === "install" && !args[1]) {
    ...
    ensureCliSymlink();
    console.log(`\nDone. Start a workflow with: railgun workflow run <name> "your task"`);
  ...
  if (group === "logs") {
    ...
    // Also support "railgun logs #3" to show events for run number 3
    if (arg && /^#\d+$/.test(arg)) {
  ...
  if (action === "install") {
    ...
    process.stdout.write(`\nStart with: railgun workflow run ${result.workflowId} "your task"\n`);
    return;
  }
  ...
  if (action === "report") {
    ...
    if (result.status === "not_found") { process.stdout.write(`${result.message}\n`); return; }
    const { run, steps } = result;
    const runLabel = run.run_number != null ? `#${run.run_number} (${run.id.slice(0, 8)})` : run.id.slice(0, 8);
    ...
    for (const s of steps) {
      if (s.completed_at && (!lastCompletedAt || s.completed_at > lastCompletedAt)) {
        lastCompletedAt = s.completed_at;
      }
    }
    let totalDuration = "-";
    if (run.created_at && lastCompletedAt) {
      const totalMs = new Date(lastCompletedAt).getTime() - new Date(run.created_at).getTime();
      if (totalMs >= 0) totalDuration = formatDuration(totalMs);
    }
    ...
    // Find max step_id length for alignment
    const maxIdLen = Math.max(...steps.map(s => s.step_id.length), 4);
    ...
    for (const s of steps) {
      ...
      const failInfo = s.status === "failed" && s.output ? ` (${s.output.slice(0, 40)})` : "";
      lines.push(`  ${s.step_id.padEnd(maxIdLen)} | ${duration.padStart(8)} | ${s.status}${failInfo}`);
    }
