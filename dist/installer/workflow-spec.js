...
export async function loadWorkflowSpec(workflowDir) {
    ...
    if (parsed.maxRunSeconds !== undefined && parsed.maxRunSeconds <= 0) {
        throw new Error(`workflow.yml maxRunSeconds must be positive in ${workflowDir}`);
    }
    if (parsed.maxStepSeconds !== undefined && parsed.maxStepSeconds <= 0) {
        throw new Error(`workflow.yml maxStepSeconds must be positive in ${workflowDir}`);
    }
    if (parsed.maxConcurrentRuns !== undefined && parsed.maxConcurrentRuns <= 0) {
        throw new Error(`workflow.yml maxConcurrentRuns must be positive in ${workflowDir}`);
    }
