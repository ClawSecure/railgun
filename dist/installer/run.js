...
export async function runWorkflow(params) {
    ...
    const activeRow = db.prepare("SELECT COUNT(*) as cnt FROM runs WHERE workflow_id = ? AND status = 'running'").get(workflow.id);
    if (activeRow.cnt >= maxConcurrent) {
        throw new Error(`Cannot start run: maximum concurrent runs (${maxConcurrent}) reached for workflow ${workflow.id}. Wait for active runs to complete.`);
    }
    ...
    const globalActiveRow = db.prepare("SELECT COUNT(*) as cnt FROM runs WHERE status = 'running'").get();
    if (globalActiveRow.cnt >= globalMaxConcurrentRuns) {
        throw new Error(`Cannot start run: global maximum concurrent runs (${globalMaxConcurrentRuns}) reached. Wait for active runs to complete.`);
    }
