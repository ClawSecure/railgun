...
import { resolveRailgunRoot, resolveWorkflowDir, resolveWorkflowWorkspaceDir, resolveWorkflowWorkspaceRoot, resolveWorkflowRoot, } from "./paths.js";
import { removeSubagentAllowlist } from "./subagent-allowlist.js";
import { uninstallRailgunSkill } from "./skill-install.js";
...
/**
 * Select only agents that are clearly Railgun-managed.
 *
 * Primary signal: agent id is prefixed by an installed Railgun workflow id.
 * Fallback signal (for partial/corrupt state): workspace is under
 * ~/.openclaw/workspaces/workflows/<workflow-id>/...
 */
export function selectRailgunManagedAgents(list, workflowIds, workflowWorkspaceRoot = resolveWorkflowWorkspaceRoot()) {
...
export async function uninstallWorkflow(params) {
    ...
    for (const entry of removedAgents) {
        ...
        // since both agent/ and sessions/ inside it are railgun-managed
...
export async function uninstallAllWorkflows() {
    ...
    const removedAgents = selectRailgunManagedAgents(list, installedWorkflowIds);
    if (config.agents) {
        config.agents.list = list.filter((entry) => !removedAgents.includes(entry));
        // Prevent an empty agents list which would brick OpenClaw (no default agent)
        if (config.agents.list.length === 0) {
            config.agents.list = [{
                    id: "main",
                    name: "Main",
    ...
    await writeOpenClawConfig(configPath, config);
    await removeMainAgentGuidance();
    await uninstallRailgunSkill();
    // Remove all railgun cron jobs
    await deleteAgentCronJobs("railgun/");
    const workflowRoot = resolveWorkflowRoot();
    if (await pathExists(workflowRoot)) {
    ...
    const railgunRoot = resolveRailgunRoot();
    if (await pathExists(railgunRoot)) {
        ...
        for (const name of ["dashboard.pid", "dashboard.log", "events.jsonl", "logs"]) {
            const p = path.join(railgunRoot, name);
            if (await pathExists(p)) {
                await fs.rm(p, { recursive: true, force: true });
            }
        }
        // Remove the directory if now empty
        const entries = await fs.readdir(railgunRoot).catch(() => ["placeholder"]);
        if (entries.length === 0) {
            await fs.rm(railgunRoot, { recursive: true, force: true });
        }
    }
